...
public class DateTimeUtil {
     ...
     * @return
     */
    public static DateTime getLastMarketDate4Stock(DateTime target){
        //判断是否是工作日
        if (isWorkDay(target)) {
            //当前日期开盘前
            if (target.isBefore(getOpenDate(target))) {
                ...
                target=target.withHourOfDay(11).withMinuteOfHour(35).withSecondOfMinute(0).withMillisOfSecond(0);
            }else if (target.isAfter(getCloseDate(target))){
                //当前日期收盘后
                target=getCloseDate(target);
            }
